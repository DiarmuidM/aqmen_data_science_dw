// File: aqmen_exercise2_20180716_dmd.do
...
/* This DO file performs the following tasks:
   ...
   Datasets:
	...
	You should consult the data dictionary throughout this exercise: http://data.charitycommission.gov.uk/data-definition.aspx
...
// Create or reuse existing macros to tell Stata where the files are
...
// Import the .csv file from the data_raw folder
...
**codebook *, problems
	...
	// Retain the'regno', 'subno' and 'orgtype' variables
	...
	tab charitystatus
	// Recode anything above 2 (the highest valid value) as missing data.
	// Create a label with the following properties: 1 "Active" 2 "Removed"
	// Label the values of 'charitystatus' with the label you created
	tab charitystatus
	// Drop the 'orgtype' variable
...
label data "One observation per main charity"
	...
	*tab1 regdate remdate
	...
	// Rename regdateyr and remdateyr to regy and remy respectively
...
codebook *, compact
	...
	// Use egen to count the number of trustees per charity; call this variable 'trustees'
	sum trustees
...
sav $path1\ew_trustees_v1.dta, replace
	...
	*/
	...
	// Merge income information from the main_charity dataset
...
/* Final data management */
	...
	/*
		Create a 'charityage' variable as follows: age = 2018 - registration year if charity is currently still active/registered;
		then replace the value of charity age as follows: age = 2018 - removal year if charity has been de-registered
	*/
...
notes
	...
	// Execute individual tabulations of the categorical variables
	...
	*/	
	...
	// Execute a table command for depvar with summary statistics for income 
	...
	// Execute a cross-tabulation of 'remy' and 'depvar' only for observations where 'remy' is greater than or equal to 2007
	
	local numobs:di %6.0fc r(N) // Creates a macro to store the number of observations from the previous tab command
